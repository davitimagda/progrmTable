...
const dayDetailLines = (day, listFilters) => {
  let teacher_lines;
  if (listFilters.teachers.length > 0) {
    teacher_lines = listFilters.teachers.map((selteacher, i) => {
      const teacher = TEACHERS.find((t) => t.name == selteacher.name);
      const line = hoursSection(day, teacher, listFilters.tmimata);
...
const teacherDetailLines = (teacher, listFilters) => {
  // console.log("teacher_MODEL");
  // console.log(teacher.getTeacherTmima(listFilters.tmimata, listFilters.days));

  const teacher_details = teacher.getTeacherTmima(listFilters);

  // const teacher_details = getTeacherTmima(
  //   teacher,
  //   listFilters.tmimata,
  //   listFilters.days
  // );
  ...
  let day_lines;
  if (listFilters.days.length > 0) {
    day_lines = listFilters.days.map((selday, i) => {
    ...
    if (Object.hasOwnProperty.call(t.days, day_aa)) {
      for (const hour in t.days[day_aa]["hours"]) {
        if (Object.hasOwnProperty.call(t.days[day_aa]["hours"], hour)) {
          ...
          // if (!tmimata[tmima]) {
          //   tmimata[tmima] = [{ day: day_aa, hour: hour }];
          // } else {
          //   tmimata[tmima].push({ day: day_aa, hour: hour });
          // }
